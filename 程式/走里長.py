...
if __name__ == '__main__':
	...
	配對結果, 無資料的候選人 = _比對里長.比對資料(_檔案所在, open(os.path.join(_這馬所在, '..', 'db_data.html')).read())
	結果檔案 = open(os.path.join(_這馬所在, '..', '結果檔案.txt'), 'w')
	for 系統內人編號, 抽著號碼, 男女 in 配對結果:
		print('{0},{1},{2}'.format(系統內人編號, 抽著號碼, 男女), file=結果檔案)
	print('無資料的候選人')
	for 選區, 選區表編號, 選區表名 in 無資料的候選人:
		print('{0}選區的{1},{2}，無資料'.format(選區, 選區表編號, 選區表名))
